...
let screenshotsLocation = "./screenshots"

async function init(screenLoc){
    screenshotsLocation = screenLoc ? screenLoc : screenshotsLocation
...
async function takeScreenshot(filesuffix,force){
    if(!screenshots && !force)
        return
    renderId++
    let name = filesuffix ? `screenshot-${renderId}-${filesuffix}.png` : `screenshot-${renderId}.png`
    await page.render(`${screenshotsLocation}/${name}`)
